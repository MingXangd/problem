...
function readrd(url, url2) {
  ...
  fs.readFile(url2, "UTF-8", function (err, dataStr) {
    ...
    // data2 = JSON.stringify(dataStr.split('export default')[1]);

    // console.log(typeof data2 ,typeof data1)
  ...
  fs.readFile(url, "UTF-8", function (err, dataStr) {
    ...
    for (let item in data2) {
      ...
      if(item == 'agreement'&&!obj[item]){
        ...
        //   // console.log(err);
        // });
        fs.writeFile(url, 'export default'+JSON.stringify(obj), function (err) {
          // console.log(err);
        });
      }
    }
